...
def get_residues_by_pos(resList, prefix_text, prefix_pdb):
	'''
		Принимает список кортежей в формате вывода strstr.loopSubstring, возвращает список списков соответствующих residue
	'''
	parser = pdb.PDBParser()
	residues = []
	regions = regions_to_dict(open(prefix_pdb + 'regions.txt', 'r').read())
	for res_tuple in resList:
		structure = parser.get_structure('strk', prefix_pdb+res_tuple[0]+'.pdb')
